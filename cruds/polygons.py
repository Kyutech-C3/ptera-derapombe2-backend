...
def calc_power_ratio(db: Session):
	...
	red = red_surface[0] if red_surface is not None else 0
	green_surface = db.query(func.sum(Polygon.surface)).filter(Polygon.group == Color.GREEN).group_by(Polygon.group).first()
	green = green_surface[0] if green_surface is not None else 0

	return PowerRatio(
		red=red/(red+green) if (red+green) > 0 else 0.5,
		green=green/(red+green) if (red+green) > 0 else 0.5
	)
